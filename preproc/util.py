...
def encoded_code_tokens_to_code_sl(encoded_tokens, indent=' '):
    decoded_tokens = []
    for i in range(len(encoded_tokens)):
        token = encoded_tokens[i]
        token = token.replace('#tab#', '\t').replace('#space#', ' ')
    ...
    code = ''.join(decoded_tokens).strip().replace(' [ ', '[ ' ).replace(' [', '[' ).replace(' ( ', '(' ).replace(' )', ')').replace(' (', '(').replace('( ', '(').replace(' ).', ').').replace('. ', '.').replace(' .', '.').replace(' . ', '.') 
    #code = ''.join(decoded_tokens).strip()

    return code

def encoded_code_tokens_to_code_batch(encoded_tokens, indent=' '):
    batch_code_snippets = []
    for single_snippet in range(len(encoded_tokens)):
        decoded_tokens = []
        for i in single_snippet:
            token = encoded_tokens[i]
            token = token.replace('#tab#', '\t').replace('#space#', ' ')
        ...
        code = ''.join(decoded_tokens).strip().replace(' [ ', '[ ' ).replace(' [', '[' ).replace(' ( ', '(' ).replace(' )', ')').replace(' (', '(').replace('( ', '(').replace(' ).', ').').replace('. ', '.').replace(' .', '.').replace(' . ', '.') 
    #code = ''.join(decoded_tokens).strip()
        batch_code_snippets.append(code)
    return batch_code_snippets

def encoded_code_tokens_to_code_sl2(encoded_tokens, indent=' '):
    decoded_tokens = []
    for i in range(len(encoded_tokens)):
        token = encoded_tokens[i]
        token = token.replace('#tab#', '\t').replace('#space#', ' ')
